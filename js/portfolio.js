  ...
  const processCases = {
    ...
    theosis: {
      ...
      layers: {
        ...
        changes: '<h2>Compile the same Church sources into the same office.</h2><p>A source manifest records authority, URL, checksum, rights, scope, and locator requirements. Importers turn the Horologion, Psalter, OCA service books, Menaion, Octoechos, Triodion, Pentecostarion, and Scripture into fixed services and variable propers. The resolver selects the office form, the service-day layer advances Vespers and Compline without changing the selected calendar route, and the composer inserts every required proper while preserving Reader or clergy roles. Publication stops on an unresolved slot, unsourced line, prohibited role, or ordinary substitution.</p><p>Python emits files for each day, month, Bible chapter, and search route. Expo exports the shared interface, and the web reader loads those files without a runtime API.</p><div class="decision-note"><strong>Decision</strong>Publish only offices whose required sections resolve from authorized sources, and keep runtime delivery read-only.</div>',
        tests: '<h2>Keep incomplete offices out of the public schedule.</h2><p>Before publication, every exposed office and role is resolved across the supported calendar range. Known calendar dates cover ordinary days, all eight tones, feasts, fasts, forefeasts, afterfeasts, leavetakings, Lent, Holy Week, Pascha, Royal Hours, and the service-day change at Vespers. The checks reject unresolved propers, unknown sources, missing locators, clergy parts in Reader services, and ordinary substitutions where another form is appointed. Browser checks open those prayer journeys on phone and desktop.</p>',
