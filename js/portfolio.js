(function () {
  ...
  const projectDepthCases = {
    ...
    'dadbod-grip': {
      ...
      panels: {
        ...
        system: '<div class="system-path system-path--four" aria-label="Dadbod Grip system cross-section"><div class="system-node"><strong>Sources</strong><span class="system-detail system-detail--wide">PostgreSQL · MySQL · SQLite · MotherDuck · local files/HTTPS</span><span class="system-detail system-detail--compact">4 databases · files/HTTPS</span></div><span aria-hidden="true">to</span><div class="system-node"><strong>DuckDB hub</strong><span class="system-detail system-detail--wide">attach databases · scan files · cross-source joins</span><span class="system-detail system-detail--compact">attach · scan · cross-source JOIN</span></div><span aria-hidden="true">to</span><div class="system-node is-core"><strong>Dadbod Grip</strong><span class="system-detail system-detail--wide">edit grids · preview SQL · atomic commit · compensating undo</span><span class="system-detail system-detail--compact">edit · preview SQL · commit · best-effort undo</span></div><span aria-hidden="true">to</span><div class="system-node"><strong>Neovim analysis</strong><span class="system-detail system-detail--wide">schemas/FKs · notebooks · profiling · Query Doctor · four AI providers</span><span class="system-detail system-detail--compact">schema · profile · notebook · AI SQL</span></div></div><p class="proof-line"><span class="system-detail system-detail--wide">One query can join attached PostgreSQL data with a local or HTTPS Parquet file. Direct-table edits use the same statement builders for preview and execution, then apply as one transaction.</span><span class="system-detail system-detail--compact">Example: JOIN PostgreSQL to Parquet. Preview generated SQL; apply as one transaction.</span></p>',
  ...
  const processCases = {
    ...
    theosis: {
      title: 'How Pray Orthodox turns Church sources into a trustworthy daily office',
      kicker: 'Case: Pray Orthodox',
      layers: {
        brief: '<h2>Make the traditional Orthodox daily cycle usable without making it less exact.</h2><p>A newcomer, parishioner, reader, chanter, or priest can open one date and find the appointed offices, Scripture, saints, and fasting rule without assembling books and PDFs by hand. The calm reading surface preserves Reader and clergy roles, calendar day and service day, fixed order and variable propers, ordinary and festal forms, and OCA New Calendar precedence.</p>',
        constraints: '<h2>One missing proper can make an otherwise complete-looking office wrong.</h2><p>Matins, Vespers, the Hours, Compline, and the Midnight Office are compositions whose form changes with weekday, tone, feast rank, fast, forefeast, afterfeast, leavetaking, Lent, Holy Week, Pascha, and the next service day. The service day begins at Vespers without moving the selected calendar page. Reader services omit priest and deacon parts, every spoken line needs provenance, and incomplete seasonal forms never fall back to an ordinary substitute. The same corpus works offline on native and loads in route-sized pieces on the public web without a live application server.</p>',
        changes: '<h2>Build a deterministic, source-audited liturgical compiler.</h2><p>A registered source manifest records authority, URL, checksum, rights, scope, and locator requirements. Deterministic importers turn the Horologion, Psalter, OCA service books, Menaion, Octoechos, Triodion, Pentecostarion, and Scripture into typed fixed services and variable proper assets. The resolver selects the office form, the service-day layer advances Vespers and Compline without changing the selected calendar route, and the composer inserts every required proper while preserving Reader or clergy roles. A full-year projection fails on an unresolved slot, unsourced line, prohibited role, or ordinary substitution.</p><p>Python also emits content-addressed day, month, Bible chapter, and search files. Expo exports the shared interface, and Cloudflare Pages serves immutable assets from the edge.</p><div class="decision-note"><strong>Decision</strong>Compile liturgical truth before deployment, fail closed on incomplete forms, and keep runtime delivery read-only.</div>',
        tests: '<h2>Prove what a person can actually open and pray, for every exposed date.</h2><p>The release gate resolves every exposed office and role across all 4,017 dates from 2025 through 2035. Independent gold dates cover ordinary days, all eight tones, feasts, fasts, forefeasts, afterfeasts, leavetakings, Lent, Holy Week, Pascha, Royal Hours, and evening service-day changes. Source and schema gates reject unresolved propers, unknown sources, missing locators, clergy parts in Reader services, and unsupported substitutions.</p><p>Immutable release <code>60a8c03c</code> passed 567 Jest tests across 53 suites, every Python, source, build, and export gate, and a live browser matrix of 179 passing journeys with three intentional skips and no failures. The stale-browser journey returns a v1 manifest, triggers one deduplicated cache-bypassing v2 fetch, and preserves the reader\'s profile and state. The unchanged matrix passed on the immutable desktop and mobile deployment and again on prayorthodox.com.</p><div class="proof-strip"><span>4,017 dates</span><span>567 tests</span><span>179 live journeys</span></div>',
        visual: '<h2>Judge the result as a prayer book, not a passing DOM.</h2><p>Phone uses one focused reading column; desktop opens into a diptych with the church day on the left and the selected office on the right. Reviews at the target phone, tablet, laptop, and desktop widths check date hierarchy, reading measure, 44-pixel controls, collapsed provenance, and horizontal overflow. Spoken prayer uses the reading hand, rubrics use distinct red italics, roles remain explicit, and explanatory text never looks like words to recite. Sources must not crowd Begin Prayer, long feast names must not dominate the civil date, and Vespers must show tomorrow\'s service day without moving today\'s calendar page.</p><p class="proof-line"><a href="https://b8eb1eb8.theosis.pages.dev/">Inspect the immutable deployment</a> · <a href="https://prayorthodox.com/">Open the live product</a></p>'
      },
