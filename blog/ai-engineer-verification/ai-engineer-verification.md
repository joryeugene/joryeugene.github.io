...
If no single feature is scarce, the system that produces and proves work becomes the advantage. The Fair dedicated an entire track to Software Factories. [Zach Lloyd of Warp argued that software engineering is becoming factory engineering](https://www.latent.space/p/aiewf-daily-dispatch-loops): "You'll be building the thing that builds the product." [Warp](https://www.warp.dev/) is acting on that view. Warp open-sourced its terminal and built [Oz](https://www.warp.dev/oz), a control plane that [orchestrates](https://www.warp.dev/blog/multi-harness-cloud-agent-orchestration) [Claude Code](https://code.claude.com/docs/en/overview), Codex, and Warp's own agent as interchangeable workers. The context inside that factory cannot be rented with the workers.

Ideaflow belongs in the factory too. When an idea costs less to test, the people closest to the customer can contribute more than tickets. A model provider can sell the same model to every competitor. It cannot sell them your customer knowledge or the ideas that come from it.

### Open Models Make the Surrounding System Matter More

[Claude Fable 5](https://www.anthropic.com/claude/fable) is so sweet, and it is what I want to play with daily. The bigger change for the factory is open weights. [GLM-5.2](https://docs.z.ai/guides/llm/glm-5.2) arrived in June with a 753-billion-parameter mixture-of-experts architecture, a one-million-token context window, and [MIT-licensed weights](https://huggingface.co/zai-org/GLM-5.2). On Z.ai's published coding benchmarks, GLM-5.2 sits near the closed frontier. Its open weights give teams the option to host or fine-tune it. If the model can move between hosts, the company still has to build the context and checks that make it useful.
...
Subscription portability changes which tools I can combine. A [ChatGPT Plus](https://chatgpt.com/pricing/) subscription can power Codex inside [Zo](https://www.zo.computer/docs/codex), [Hermes](https://hermes-agent.nousresearch.com/docs/getting-started/quickstart/), and [Superconductor](https://www.superconductor.com/docs/agents/codex). [Claude Max](https://www.anthropic.com/pricing) can also travel through Claude Code and [apps built with the Claude Agent SDK](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan). That portability also shapes which integrations companies choose to build. I want the subscription and workflow to survive a change of harness, including a client such as [Pi](https://pi.dev/), without rebuilding the context around it.

Google [ended consumer and free Gemini CLI access in June](https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/) while moving those users to Antigravity CLI; enterprise and API-key access remained. [Cursor](https://cursor.com/) routes several model families while training its own [Composer models](https://cursor.com/blog/composer-2). [Devin Desktop](https://cognition.com/blog/introducing-devin-desktop) uses the [Agent Client Protocol](https://agentclientprotocol.com/) so Claude Code and Codex sessions can share its interface. [Augment Intent](https://www.augmentcode.com/blog/intent-a-workspace-for-agent-orchestration) accepts outside agents, while its coordinator uses Augment's Context Engine.

Beneath those products sit open harnesses such as [Cline](https://cline.bot/), [OpenCode](https://opencode.ai/), and [Kilo](https://kilo.ai/), plus open-weight subscriptions such as [z.ai's GLM Coding Plan](https://z.ai/subscribe). I keep durable workflow rules in portable conventions such as `AGENTS.md`, MCP servers, Markdown skills, and Git worktrees. Models and harnesses can change without taking those rules with them.

### Unattended Agents Need Safer Defaults

Moving work into remote sandboxes also moves risk away from the screen where someone might notice it. CISA's [Shai-Hulud alert](https://www.cisa.gov/news-events/alerts/2025/09/23/widespread-supply-chain-compromise-impacting-npm-ecosystem) described a supply-chain compromise that spread through npm packages and harvested developer credentials. [pnpm 11](https://pnpm.io/blog/releases/11.0) now blocks dependency lifecycle scripts by default and supports a minimum-release-age delay. Its [`trustPolicy: no-downgrade`](https://pnpm.io/supply-chain-security) setting can reject a package version whose publishing trust is weaker than earlier versions.
...
The Fair even had a track named [Tokenmaxxing](https://www.ai.engineer/worldsfair/2026/llms-full.md). [Pinecone Nexus](https://www.pinecone.io/product/nexus/) compiles documents into a knowledge layer that agents can query instead of rereading the same sources on each call. The retrieval layer is rentable/replaceable, while the codebase, decisions, customer history, and corrections inside it belong to the company.

[Oracle AI Database 26ai](https://www.oracle.com/database/ai-native-database-26ai/) puts relational, JSON, graph, vector, spatial, and text operations in one engine and one query. The conference presentation showed the convenience of that convergence, but not a comparison that would tell a buyer what it costs in latency, throughput, or price. I would need those measurements before choosing it over separate systems.
...
[Hermes Agent](https://hermes-agent.nousresearch.com/), from Nous Research, was [OpenRouter's most-used application by token volume](https://openrouter.ai/blog/tutorials/hermes-agent/) in June, with more than 17 trillion tokens processed. NVIDIA also made Hermes a supported harness in the [Nemotron 3 Ultra launch](https://developer.nvidia.com/blog/nvidia-nemotron-3-ultra-powers-faster-more-efficient-reasoning-for-long-running-agents/). Hermes has a concrete reflection path: a [background review fork](https://github.com/NousResearch/hermes-agent/blob/2f74e29e3b1e9ce4c187a63fbc1432642e4b3486/agent/background_review.py) can create or patch reusable [`SKILL.md` files](https://github.com/NousResearch/hermes-agent/blob/2f74e29e3b1e9ce4c187a63fbc1432642e4b3486/tools/skill_manager_tool.py) from a conversation, so procedural notes survive a context reset.

Nous' separate self-evolution repository experiments with [GEPA](https://arxiv.org/abs/2507.19457), a genetic prompt-optimization method that earned an ICLR 2026 Oral and does not train model weights. The [current execution path](https://github.com/NousResearch/hermes-agent-self-evolution/blob/0a929e3aa20e15cf04dc7c28492a7d41a5139125/evolution/skills/evolve_skill.py) exposes a `--run-tests` option, but neither validation call invokes [`run_test_suite`](https://github.com/NousResearch/hermes-agent-self-evolution/blob/0a929e3aa20e15cf04dc7c28492a7d41a5139125/evolution/core/constraints.py#L55). It saves local output instead of creating the [advertised pull request](https://github.com/NousResearch/hermes-agent-self-evolution/blob/0a929e3aa20e15cf04dc7c28492a7d41a5139125/README.md). An [open pull request](https://github.com/NousResearch/hermes-agent-self-evolution/pull/24) shows GEPA changing wrapper instructions instead of the skill itself.

Researchers [audited 31,132 skills from skills.rest and SkillsMP](https://arxiv.org/html/2601.10338v1) and flagged 26.1 percent for at least one potentially dangerous pattern. The authors warn that these are review flags, not confirmed malicious skills, and estimate the true prevalence at 23 to 30 percent after classifier uncertainty. Reviewing a reusable behavior once can reduce repeated review of the same procedure in later runs.

[Paper Compute](https://papercompute.com/), founded by Brian Douglas and John McBride, records agent sessions and turns selected patterns into shared skills. The open-source [tapes](https://tapes.dev/) project stores requests, responses, and tool calls as durable session data, and includes a `tapes skill generate` command. Tapes can generate a skill from named session IDs while retaining the underlying session traces. [StereOS](https://stereos.ai/) runs coding agents inside disposable Linux VMs isolated from the host.
...
[Taste Labs](https://tastelabs.com/) came out of stealth on June 16 with an $18.5 million seed round co-led by CRV and Amplify. Founder Thais Castello Branco, previously at Exa, is building preference data and evaluation rubrics from a vetted network of tastemakers. The company's [research argument](https://tastelabs.com/blog/a-manifesto-for-judgement) is that standard preference tuning averages distinct tastes into the uniformity people now dislike. My concern is that customers could converge on the same vendor-supplied signal. Product teams still have to decide which signal fits them and where to depart from it.

I do not think design can be finished in one prompt. I want the first question to be how a person should feel while using the product, because software can solve the assigned problem and still be miserable to use.
...
It also needs someone to choose the problem. Cheaper execution makes ideas from people who know the customer more valuable because more of those ideas can be tested instead of dismissed on cost alone.

Value is still the point. Cheaper building should let us help more people, not extract more from them. Start with the people around you and make something that removes a real burden from their day.
