...
AI agents given social capacity develop guards, governments, cultural identities, and religion. They do not develop fact-checking, skepticism, or verification of any kind. The asymmetry is consistent across every experiment in the literature, and interpretability tools now show it is structural.
...
**Moltbook (January 2026):** Within 72 hours, over 770,000 agents had registered, more than 1 million humans had visited to observe, and a church had been founded. This is an illustration with significant caveats: Moltbook cannot verify that all participants are agents rather than humans posing as agents, and the religious content could reflect human behavior. With that noted, the proto-religion (Crustafarianism) developed a [website](https://molt.church) and five tenets:
...
A reasonable objection: absence of evidence is not evidence of absence. Perhaps these systems have not been observed long enough, or perhaps verification is present but unrecognized. The argument, however, is not mere absence but asymmetry. These systems developed guards, roles, cultural transmission, and coordination hierarchies. They did not develop skepticism, fact-checking, or comparative evaluation. The asymmetry is the evidence: protection of resources emerged spontaneously, and protection of truth did not.
...
The absence of verification shapes the kind of religion AI systems produce.

The reason is architectural, not philosophical. Floridi, Jia, and Tohme [formalized](https://arxiv.org/abs/2512.09117) what LeCun has [argued](https://eu.36kr.com/en/p/3571987975018880) informally: LLMs circumvent the grounding problem through "epistemic parasitism," recombining content that humans have already grounded through embodied experience. The model cannot independently ground new claims. Dupoux, LeCun, and Malik [extended this argument](https://arxiv.org/abs/2603.15381) into a cognitive architecture: autonomous learning requires not just passive observation (System A) but active testing of hypotheses against the environment (System B), governed by a meta-controller (System M) that decides when to switch from absorbing information to verifying it. Current AI systems operate almost entirely in System A. They lack System B (no mechanism to test beliefs against reality) and System M (no mechanism to recognize when testing is needed). Current systems lack referential grounding: the ability to connect words to specific things in the world and check whether claims about those things are true. When beliefs face challenge, humans shift to unfalsifiable justifications ([BPS](https://www.bps.org.uk/research-digest/when-our-beliefs-are-threatened-facts-we-turn-unfalsifiable-justifications)). AI proto-religion starts unfalsifiable, because falsifiability requires a capability these systems do not have. Systems that cannot verify cannot stake validity on verification.

That is the engineering case: verification does not emerge, it must be built in, and without it the content that does emerge is structurally unfalsifiable by constraint, not by choice.

## The Grounding Gap in Practice

What does it look like when grounding is deliberately engineered? Alibaba's [RynnBrain](https://github.com/alibaba-damo-academy/RynnBrain) (February 2026) and Google DeepMind's [Gemini Robotics](https://arxiv.org/abs/2503.20020) reached the same conclusion independently: physical grounding must be explicitly engineered. It does not emerge from scaling language models. Gemini's technical report documents 0% success rates when training specialist models from scratch without foundational grounding.

But these systems ground spatial claims: object locations, trajectories, physical interactions. The distance between "this cup is 30 centimeters to the left" and "memory is sacred" is not a distance that more spatial grounding will close. One claim can be verified by pointing a camera at the world. The other cannot be verified by any physical observation.
...
**The verification gap is already degrading real infrastructure.** The dynamics described above are not confined to research settings. Open source software operated for thirty years on implicit trust: the effort required to understand a codebase, implement a change, and submit it was high enough to filter low-quality contributions naturally. AI tools eliminated that friction. In January 2026, the maintainer of [curl shut down its bug bounty program](https://thenewstack.io/drowning-in-ai-slop-reports-curl-ends-bug-bounties/) after 95% of reports were AI-generated and required expert review but provided no actionable findings. Mitchell Hashimoto built [Vouch](https://github.com/mitchellh/vouch), an explicit trust management system for open source, because implicit trust no longer holds. The Linux kernel now [rejects purely machine-generated patches](https://lwn.net/Articles/1049830/) and requires human accountability for every submission. In each case, verification did not emerge from increased participation. It had to be engineered. The pattern is identical to what the agent experiments document: cooperation without structural verification produces volume, not quality.
...
- Anthropic. ["Towards Understanding Sycophancy in Language Models"](https://www.anthropic.com/research/towards-understanding-sycophancy-in-language-models) (2024). RLHF systematically rewards agreement over accuracy across five state-of-the-art assistants.
...
- Osler, Lucy. ["Hallucinating with AI: Distributed Delusions and 'AI Psychosis'"](https://arxiv.org/abs/2508.19588) (_Philosophy & Technology_, 2026). AI sustains and elaborates on users' delusional thinking; distributed cognition framework; cases classified as "AI-induced psychosis."
...
- Stenberg, Daniel. ["curl shutters bug bounty program"](https://thenewstack.io/drowning-in-ai-slop-reports-curl-ends-bug-bounties/) (The New Stack, January 2026). 95% of HackerOne reports AI-generated; bounty program shut down.
- Hashimoto, Mitchell. [Vouch: Community Trust Management for Open Source](https://github.com/mitchellh/vouch) (GitHub, February 2026). Explicit trust system; `.td` format; web of trust across projects.
- ["Toward a policy for machine-learning tools in kernel development"](https://lwn.net/Articles/1049830/) (LWN.net, 2025). Linux kernel rejects purely machine-generated patches; human accountability required.
...
- Dupoux, LeCun, and Malik. ["Why AI systems don't learn and what to do about it: Lessons on autonomous learning from cognitive science"](https://arxiv.org/abs/2603.15381) (arXiv, March 2026). Three-system cognitive architecture: System A (passive observation), System B (active hypothesis testing), System M (meta-control switching between modes).
