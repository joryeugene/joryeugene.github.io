...
async function state(page) {
  return await page.evaluate(() => {
    const mode = document.querySelector('#vim-status-mode')?.textContent?.trim();
    const pos = document.querySelector('#vim-status-pos')?.textContent?.trim();
    const file = document.querySelector('#vim-status-file')?.textContent?.trim();
    return { mode, pos, file };
  });
}

async function clearToEmpty(page) {
  await cmd(page, 'enew');
}

async function seed(page, text) {
  await clearToEmpty(page);
  await press(page, 'i');
  await type(page, text);
  await press(page, 'Escape');
  // After Escape the cursor is on the last typed character. Park it at (1,0)
  // so motion tests start from a known position. :enew leaves a single empty
  // line prefix, and on some flows gg alone may land on col > 0; force col 0.
  await press(page, 'g');
  await press(page, 'g');
